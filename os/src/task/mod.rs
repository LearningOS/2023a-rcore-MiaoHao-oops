...
impl TaskManager {
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            inner.tasks[next].cur_time = get_time_ms();
    ...
    fn get_running_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].cur_time - inner.tasks[current].begin_time
    }
}
...
/// Update syscall number array of current running task.
pub fn update_syscall_num(syscall_id: usize) {
    TASK_MANAGER.update_syscall_num(syscall_id);
}

/// Get syscall number array of current running task.
